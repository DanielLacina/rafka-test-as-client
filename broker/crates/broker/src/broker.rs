...
use rafka_core::proto::rafka::{
    ...
    AcknowledgeRequest, AcknowledgeResponse, ConsumeRequest, ConsumeResponse, GetMetricsRequest,
    GetMetricsResponse, PublishRequest, PublishResponse, RegisterRequest, RegisterResponse,
    SubscribeRequest, SubscribeResponse, UpdateOffsetRequest, UpdateOffsetResponse,
};
use rafka_storage::db::{RetentionPolicy, Storage, StorageMetrics};
...
impl Broker {
    ...
    async fn ensure_topic(&self, topic: &str) {
        ...
        if !topics.contains_key(topic) {
            ...
            if !topics.contains_key(topic) {
                ...
                self.storage
                    .create_partition(topic, self.partition_id as i32);
            }
        }
    }

    async fn _publish_internal(
        &self,
        response: ConsumeResponse,
    ) -> Result<(), broadcast::error::SendError<ConsumeResponse>> {
    ...
    async fn _get_consumer_offset(&self, consumer_id: &str, topic: &str) -> i64 {
        ...
        offsets
            .get(&(consumer_id.to_string(), topic.to_string()))
            .copied()
            .unwrap_or(-1)
    }
    ...
    pub fn update_retention_policy(&self, max_age: Duration, max_bytes: usize) {
        self.storage
            .update_retention_policy(RetentionPolicy { max_age, max_bytes });
    }
...
impl BrokerService for Broker {
    async fn publish(
        ...
        println!(
            "Message published to partition {} with offset {}",
        ...
        Ok(Response::new(PublishResponse {
            ...
            success: true,
            message: format!(
                "Published successfully to partition {} with offset {}",
                self.partition_id, offset
            ),
    ...
    async fn consume(
        ...
        let req = request.into_inner();
        println!(
            "Consumer {} started consuming on partition {}",
            req.id, self.partition_id
        );
    ...
    async fn acknowledge(
        ...
        let req = request.into_inner();
        println!(
            "Acknowledged message: {} for consumer {}",
            req.message_id, req.consumer_id
        );
    ...
    async fn update_offset(
        ...
        self.set_consumer_offset(&req.consumer_id, &req.topic, req.offset)
            .await;

        println!(
            "Updated offset for consumer {} on topic {} to {}",
